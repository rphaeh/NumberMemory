...
class QGToast: NSObject {
    ...
    var frameSize:CGSize = CGSize(width: screenWidth - 40, height: 265)
    var view:UIView!
    ...
    init(text:String,duration:TimeInterval) {
        ...
        let label:UILabel = UILabel (frame: CGRect(x: 0, y: 0, width: size.width + 20, height: size.height))
        label.text = text
        ...
        let v:UIButton = UIButton(frame:CGRect(x: 0, y: 0, width: size.width + frameMarginSize + 20, height: size.height + frameMarginSize))
        ...
        v.addTarget(self, action: #selector(QGToast.hideToast), for: .touchDown)
    ...
    func sizeWithString(_ string:NSString, font:UIFont)-> CGSize {
        ...
        let rect:CGRect = string.boundingRect(with: frameSize, options:options, attributes: [NSAttributedString.Key(rawValue: "NSFontAttributeName"): font], context: nil)
        return rect.size
    }
