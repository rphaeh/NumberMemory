...
class RootNavigationController: UINavigationController {
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if self.children.count > 0 {
            ...
            //导航栏返回按钮自定义
            let backButton = UIButton(frame:CGRect.init(x: 0, y: 2, width: 50, height: 33))
            backButton.setImage(UIImage.init(named: "white_back"), for: .normal)
            
            backButton.addTarget(self, action:#selector(self.didBackButton(sender:)), for: .touchUpInside)
//            backButton.sizeToFit()
//            backButton.backgroundColor = .red
            backButton.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.left //居左显示
